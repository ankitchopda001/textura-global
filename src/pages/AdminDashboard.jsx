...
function AdminDashboard() {
  ...
  const exportToExcel = () => {
    ...
    XLSX.writeFile(workbook, "Textura_Global_Inquiries.xlsx");
  };
  ...
  const todayInquiries = inquiries.filter(
    (item) =>
      item.created_at && new Date(item.created_at).toDateString() === today
  ).length;

  const totalEmails = new Set(
    inquiries.map((item) => item.email).filter(Boolean)
  ).size;

  const totalPhones = new Set(
    inquiries.map((item) => item.phone).filter(Boolean)
  ).size;
  ...
  const filteredInquiries = inquiries.filter((item) => {
    ...
    if (item.created_at) {
      ...
      if (dateFilter === "Today") {
        matchesDate = inquiryDate.toDateString() === todayDate.toDateString();
      }

      if (dateFilter === "Week") {
        const firstDay = new Date(todayDate);
        firstDay.setDate(todayDate.getDate() - 7);
        matchesDate = inquiryDate >= firstDay;
      }

      if (dateFilter === "Month") {
        matchesDate =
          inquiryDate.getMonth() === todayDate.getMonth() &&
          inquiryDate.getFullYear() === todayDate.getFullYear();
      }
    }
  ...
  return (
    <div className="min-h-screen bg-slate-950 text-white p-8">
      ...
      <motion.div
        ...
        className="flex justify-between items-center mb-10 mt-6"
      >
        <button
